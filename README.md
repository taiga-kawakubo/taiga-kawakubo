<!-- GitHub profile README for taiga-kawakubo -->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,45:2563eb,100:14b8a6&height=210&section=header&text=Taiga%20Kawakubo&fontSize=54&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Laravel%20%2F%20PHP%20Developer%20%7C%20Web%20Applications%20%26%20Learning%20AI%20Agents&descSize=18&descAlignY=58" alt="Taiga Kawakubo - Laravel / PHP Developer, Web Applications and Learning AI Agents" width="100%" />

[![Profile views](https://komarev.com/ghpvc/?username=taiga-kawakubo&label=Profile%20views&color=2563eb&style=flat-square)](https://github.com/taiga-kawakubo)
[![Followers](https://img.shields.io/github/followers/taiga-kawakubo?label=Followers&style=flat-square&color=2563eb)](https://github.com/taiga-kawakubo?tab=followers)
[![Following](https://img.shields.io/badge/dynamic/json?color=2563eb&label=Following&style=flat-square&query=%24.following&url=https%3A%2F%2Fapi.github.com%2Fusers%2Ftaiga-kawakubo)](https://github.com/taiga-kawakubo?tab=following)
[![Public repos](https://img.shields.io/badge/dynamic/json?color=2563eb&label=Public%20repos&style=flat-square&query=%24.public_repos&url=https%3A%2F%2Fapi.github.com%2Fusers%2Ftaiga-kawakubo)](https://github.com/taiga-kawakubo?tab=repositories)

</div>

## <img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="28" /> About Me

Laravel / PHPを中心に、認証・認可、REST API、外部API連携、バッチ処理、通知、テストまで含めたWebアプリケーション開発に取り組んでいます。

代表作のBookShelfでは、書籍・レビュー・お気に入り・読書計画などの機能に加え、Sanctumを使用したAPI認証、Policyによる認可、Schedulerによる通知処理を実装しました。

また、ChatGPTやCodexを開発に活用するだけでなく、質問分類、学習記録、振り返り支援を行うプログラミング学習用AI Agentの設計・改善にも取り組んでいます。

```yaml
name: Taiga Kawakubo
role: Laravel / PHP Web Developer
focus:
  - Web application development
  - REST API / Authentication / Authorization / Testing
  - Learning-support AI Agent development
currently_building:
  - BookShelf
  - Programming Study AI Agent
goal: "基礎を固め、実務で価値を出せるエンジニアになる"
```

- Notionに学習ログを残し、理解したこと・詰まった点・改善点を振り返りながら学習しています。
- ChatGPT / CodexなどのAIを活用し、より良いコードや設計を考えながら実装しています。
- 学習者が自分で読み、考え、確認する力を育てられるAI Agentを開発しています。

## 🧰 Tech Stack

<details open>
<summary><b>Languages</b></summary>
<br />
<p>
  <img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white" alt="PHP" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" alt="JavaScript" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML5" />
  <img src="https://img.shields.io/badge/CSS3-663399?style=for-the-badge&logo=css&logoColor=white" alt="CSS3" />
</p>
</details>

<details open>
<summary><b>Frameworks & Libraries</b></summary>
<br />
<p>
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" alt="Laravel" />
  <img src="https://img.shields.io/badge/Blade-F05340?style=for-the-badge&logo=laravel&logoColor=white" alt="Blade" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" alt="Tailwind CSS" />
  <img src="https://img.shields.io/badge/Alpine.js-8BC0D0?style=for-the-badge&logo=alpinedotjs&logoColor=black" alt="Alpine.js" />
</p>
</details>

<details open>
<summary><b>Backend & Database</b></summary>
<br />
<p>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" alt="MySQL" />
  <img src="https://img.shields.io/badge/Eloquent_ORM-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" alt="Eloquent ORM" />
  <img src="https://img.shields.io/badge/REST_API-009688?style=for-the-badge" alt="REST API" />
  <img src="https://img.shields.io/badge/Laravel_Fortify-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" alt="Laravel Fortify" />
  <img src="https://img.shields.io/badge/Laravel_Sanctum-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" alt="Laravel Sanctum" />
</p>
</details>

<details open>
<summary><b>Testing & Environment</b></summary>
<br />
<p>
  <img src="https://img.shields.io/badge/PHPUnit-3C9CD7?style=for-the-badge&logo=php&logoColor=white" alt="PHPUnit" />
  <img src="https://img.shields.io/badge/Laravel_Pint-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" alt="Laravel Pint" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker" />
  <img src="https://img.shields.io/badge/Laravel_Sail-FF2D20?style=for-the-badge&logo=laravel&logoColor=white" alt="Laravel Sail" />
</p>
</details>

<details open>
<summary><b>Tools & AI</b></summary>
<br />
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" alt="Git" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  <img src="https://img.shields.io/badge/Codex-000000?style=for-the-badge&logo=openai&logoColor=white" alt="Codex" />
  <img src="https://img.shields.io/badge/ChatGPT-10A37F?style=for-the-badge&logo=openai&logoColor=white" alt="ChatGPT" />
  <img src="https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white" alt="Notion" />
</p>
</details>

## 🚀 Featured Projects

### BookShelf

Laravel 10で開発した、書籍レビューと読書習慣を管理するWebアプリケーションです。<br />
書籍検索・レビュー・お気に入り・読書計画などの利用者向け機能に加え、API認証・認可、通知、日次バッチ、テストまで実装しています。

| Item | Detail |
| :--- | :--- |
| Repository | [bookshelf-app](https://github.com/taiga-kawakubo/bookshelf-app) |
| Main Features | 書籍検索・登録 / レビュー / お気に入り / いいね / 読書計画 / 通知 / ランキング |
| Backend | Fortify / Sanctum / Policy / REST API / Google Books API / Laravel Scheduler |
| Quality | Feature・Unitテスト / Laravel Pint / Route設計書 / ER図 / 画面遷移図 |
| Stack | Laravel 10 / PHP / Blade / Tailwind CSS / Alpine.js / MySQL / Docker |

### Programming Study AI Agent

Codex上で動作する、プログラミング学習を支援するAI Agentです。<br />
すぐに正解を提示するのではなく、学習者が自分でコードを読み、考え、確認する力を育てることを目的に設計しています。

| Item | Detail |
| :--- | :--- |
| Repository | [Programming-study-ai-agent](https://github.com/taiga-kawakubo/Programming-study-ai-agent) |
| Main Features | 質問分類 / 段階的な説明 / 理解確認 / Learning Case / 学習ログ / 学習傾向の整理 |
| Design | 確認範囲の制御 / 保存前の承認 / Human-in-the-loop / 継続テーマの管理 |
| Stack | Codex / Markdown / Rubyによる設定・分類ラベルの整合性検証 |

## 📊 GitHub Activity

<div align="center">

<img src="./profile/streak.svg" alt="GitHub contribution streak" width="100%" />

</div>

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/taiga-kawakubo/taiga-kawakubo/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/taiga-kawakubo/taiga-kawakubo/output/github-contribution-grid-snake.svg" />
  <img alt="Snake animation eating GitHub contributions" src="https://raw.githubusercontent.com/taiga-kawakubo/taiga-kawakubo/output/github-contribution-grid-snake.svg" />
</picture>

</div>

## Contact

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-taiga--kawakubo-181717?style=for-the-badge&logo=github)](https://github.com/taiga-kawakubo)
[![Qiita](https://img.shields.io/badge/Qiita-taiga--kawakubo-55C500?style=for-the-badge&logo=qiita&logoColor=white)](https://qiita.com/taiga-kawakubo)

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,45:2563eb,100:14b8a6&height=110&section=footer" alt="Footer" width="100%" />
